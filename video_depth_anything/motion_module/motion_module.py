...
class TemporalAttention(CrossAttention):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None, video_length=None):
        ...
        if use_memory_efficient:
            ...
            # 使用与 xformers 路径相同的 reshape 方法，确保张量形状一致
            query = self.reshape_heads_to_4d(query)
            key = self.reshape_heads_to_4d(key)
            value = self.reshape_heads_to_4d(value)
        ...
        # 当前我们面临两个问题：
        # 1. use_memory_efficient 路径和非 use_memory_efficient 路径需要适当统一
        # 2. 确保使用正确的形状处理方法

        use_memory_efficient = XFORMERS_AVAILABLE and self._use_memory_efficient_attention_xformers and query.device.type == 'cuda'

        if use_memory_efficient:
            # xformers 路径使用 reshape_heads_to_4d 将张量转换为 4D [batch, heads, seq_len, dim_per_head]
            query = self.reshape_heads_to_4d(query)
            key = self.reshape_heads_to_4d(key)
            value = self.reshape_heads_to_4d(value)
            ...
            # 现在 hidden_states 应该是 4D 形状 [batch, heads, seq_len, dim_per_head]
            # 我们需要将其转换为 3D 才能传给 to_out[0] 线性层
            batch_size, num_heads, seq_len, head_dim = hidden_states.shape
            hidden_states = hidden_states.permute(0, 2, 1, 3).reshape(batch_size, seq_len, num_heads * head_dim)
            
        else:
            # 非 xformers 路径使用 reshape_heads_to_batch_dim 将张量转换为 [batch*heads, seq_len, dim_per_head]
            ...
            # 在非 xformers 路径中，_attention 返回的是 [batch*heads, seq_len, dim_per_head] 形状
            # 这需要由 reshape_batch_dim_to_heads 转换回 [batch, seq_len, dim] 形状
